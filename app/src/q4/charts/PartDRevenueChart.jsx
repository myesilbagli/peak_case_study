...
import { daysPartD, cumRevenueA_Spreadsheet, cumRevenueB_Spreadsheet } from '../../data/q4SpreadsheetData';

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend, Filler);

export function PartDRevenueChart() {
  const { theme } = useTheme();
  const colors = getChartColors(theme);
  const data = {
    labels: daysPartD,
    datasets: [
      { label: 'Variant A', data: cumRevenueA_Spreadsheet, borderColor: colors.navy, backgroundColor: colors.navyRgba, fill: true, tension: 0.2, pointRadius: 0 },
      { label: 'Variant B', data: cumRevenueB_Spreadsheet, borderColor: colors.red, backgroundColor: colors.redRgba, fill: true, tension: 0.2, pointRadius: 0 },
    ],
  };
  const options = {
    responsive: true,
